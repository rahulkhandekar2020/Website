...
glimpse(data_1)
glimpse(data_2)
data_2 %>% dplyr::inner_join(data_1, by="ï..ID") %>% na.omit()
data_3 <- data_2 %>% inner_join(data_1, by="ï..ID") %>% na.omit()
glimpse(data_3)
data_3 %>% select(ï..ID,sport, rcc) %>% pivot_wider(names_from="sport", values_from="rcc")
data_4 <- data_3 %>% pivot_wider(names_from="sport", values_from="rcc")
data_4<-data_4 %>% pivot_longer(13: 22, names_to = "sport", values_to = "rcc",values_drop_na = T)
n_distinct(data_4)
data_4 %>% summarize_if(is.numeric,c(max=max,min=min,mean=mean,median=median))%>%pivot_longer(everything())%>%
  separate(name,into=c("var","stat"))%>%pivot_wider(names_from="stat",values_from = "value")
